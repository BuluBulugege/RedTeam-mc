...
[![Modules](https://img.shields.io/badge/Modules-100%2B-red)](src/main/java/com/redblue/red/modules)
...
**RED TEAM** is a *client-side Minecraft Forge mod that demonstrates security vulnerabilities in popular mod ecosystems*. It provides 100+ modular exploit implementations targeting missing server-side validation, rate limiting bypasses, NBT injection, packet spoofing, and DoS vectors across 20+ widely-used Minecraft mods — all through a clean GUI with hotkey bindings and remote code loading.
...
<div align="center"><img src="public/illus-module-architecture.en.png" width="600" /></div>

1. `RedMod` registers all 100+ built-in `AttackModule` implementations into `ModuleRegistry` at startup
2. Modules are grouped by target mod prefix (e.g. `lrt`, `tacz`, `ia`, `ftq`)
...
<div align="center"><img src="public/illus-exploit-flow.en.png" width="600" /></div>

1. Player opens the GUI with **K** or triggers a hotkey slot (**Shift+1–0**)
2. `KeyDispatcher` routes the input — opens `RedMainScreen` or calls `module.execute()` / `module.tick()`
3. The module constructs a crafted packet payload targeting the vulnerable mod's channel
4. `PacketForge` sends the packet — raw mode first, reflection fallback if needed
5. The server-side mod processes the packet without validation → exploit succeeds

Phase 3 — Stealth System

<div align="center"><img src="public/illus-stealth-system.en.png" width="600" /></div>

1. On `FMLLoadCompleteEvent`, `HideOnLoad` triggers both hiders
2. `ModHider` uses reflection to remove the mod from Forge's `indexedMods`, `modFiles`, `sortedList`, and related structures
3. `HandshakeHider` cleans `NetworkRegistry.instances` and `NetworkRegistry.channels`
4. Remote modules load via `InMemoryClassLoader` — no JAR written to disk, no file traces
...
| Flavor Immersed Daily | FID01–03 | Remote craft, batch exploit, remote GUI |
| FTB Quests | FTQ01–06 | Item dupe, quest vandal, structure leak |
| Legendary Survival | LSO01–02 | Infinite heal, instant drink |
| Item Filters | IF01–02 | Filter swap, ReDoS |
| Armourer's Workshop | AW01–06 | Wardrobe hijack, skin destroy, NBT inject |
| PingWheel | PW01–03 | Ping spoof, channel spy, flood |
...
# Output JAR
build/libs/redteam-1.0.0.jar
```

Install into your Minecraft 1.20.1 + Forge 47.2.0 client mods folder.
...
[![Forge](https://img.shields.io/badge/Forge-47.2.0-orange)](https://minecraftforge.net)
[![Java](https://img.shields.io/badge/Java-17-blue)](https://openjdk.org)
[![模块数量](https://img.shields.io/badge/模块-100%2B-red)](src/main/java/com/redblue/red/modules)
</div>

**RED TEAM** 是一个客户端 Minecraft Forge Mod，用于演示主流 Mod 生态中的安全漏洞。提供 100+ 模块化漏洞利用实现，覆盖 20+ 广泛使用的 Minecraft Mod，涉及缺失的服务端验证、速率限制绕过、NBT 注入、数据包伪造和 DoS 向量。
...
<div align="center"><img src="public/illus-module-architecture.zh.png" width="600" /></div>
...
<div align="center"><img src="public/illus-exploit-flow.zh.png" width="600" /></div>
...
<div align="center"><img src="public/illus-stealth-system.zh.png" width="600" /></div>
...
将 JAR 安装到 Minecraft 1.20.1 + Forge 47.2.0 客户端 mods 文件夹。
